...
export function AddContactDialog({ open, onOpenChange, onSaved }: AddContactDialogProps) {
  ...
  async function onSubmit(values: FormValues) {
    const result = await sendInvitation({
      input: { type: "CONTACT", email: values.email.trim(), targetId: null },
    });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              They must have signed in to FinPlan at least once, and will need to accept your request.
            </p>
          </div>
          <DialogFooter>
